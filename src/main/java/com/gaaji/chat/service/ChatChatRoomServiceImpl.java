...
public class ChatChatRoomServiceImpl implements ChatRoomService {
    ...
    public RoomResponseDto findRoomByRoomId(String ownerId, String roomId) {
        ...
        groupChatMemberRepository.findByMemberAndChatRoom(user, chatRoom).orElseThrow(NotYourRoomException::new);
    ...
    public RoomResponseDto saveRoomForUser(String userId, RoomSaveRequestDto dto) {
        ...
        groupChatMemberRepository.save(ChatRoomMember.create(UUID.randomUUID().toString(), user, chatRoom));
        for (RoomSaveRequestDto.UserDto member : dto.getMembers()) {
            ...
            groupChatMemberRepository.save(ChatRoomMember.create(UUID.randomUUID().toString(), memberUser, chatRoom));
        }
        return RoomResponseDto.of(chatRoom);
    }
    ...
    public List<RoomResponseDto> findRoomsByUserId(String userId) {
        ...
        for (ChatRoomMember chatRoomMember : user.getChatRoomMembers()) {
            list.add(RoomResponseDto.of(chatRoomMember.getChatRoom()));
        }
        return list;
    }
}
